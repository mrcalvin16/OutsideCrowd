...
const flyerTemplates = [
  {
    ...
    prompt: "luxury nightlife party with stylish crowd, premium venue, event image inspiration, velvet rope energy",
    eventType: "nightlife",
  ...
  },
  {
    ...
    prompt: "Afrobeats party with vibrant dance energy, stylish guests, premium cultural nightlife",
    eventType: "nightlife",
    composition: "hero",
  ...
  },
  {
    ...
    prompt: "rooftop social event with skyline views, champagne atmosphere, elevated lifestyle",
    eventType: "brunch",
  ...
  },
  {
    ...
    prompt: "outdoor music festival with stage lights, crowd energy, confetti, live performance",
    eventType: "festival",
    composition: "chaotic",
...
export default function FlyerStudioPage() {
  ...
  const [variations, setVariations] = useState<{ id: string; imageUrl: string; caption?: string }[]>([]);
  ...
  }, [savedCreative, selectedCreativeId, prompt, imagePreview, selectedEventId]);
...
const imageUrl =
  ...
  (data.imageBase64
    ...
    : "");
      ...
      const convexImageUrl = await fetch(
        `/api/convex-image-url?storageId=${storageId}`
      ).then((r) => r.text());

      // Use Convex CDN URL instead of base64
      setImagePreview(convexImageUrl);
  ...
  style,
        ...
        title: creativePackName || `${eventType} ${format} flyer`,
  ...
  async function handleSaveToEvent() {
    ...
    if (existingCreative) {
      await updateCreative({
        creativeId: existingCreative._id,
              ...
              <div className="flex items-center justify-between gap-4">
                <div>
                  ...
                  <p className="mt-1 text-xs font-bold uppercase tracking-[0.2em] text-white/35">
                    {selectedCreativeId ? "Editing Saved Draft" : "New Creative Draft"}
                  </p>
                </div>
              ...
              <div className="mt-6 space-y-5">
                ...
                {!selectedEvent && (
                  <p className="rounded-3xl border border-orange-300/20 bg-orange-500/10 p-4 text-sm font-bold text-orange-100">
                    Select an event first so your generated flyer can be saved automatically.
                  </p>
                )}
              ...
              {variations.map((variation, index) => (
                ...
                >
                  <img
                    ...
                    alt={variation.caption || `Flyer variation ${index + 1}`}
            ...
            <section className="space-y-6">
              ...
              <div className="oc-card p-5 sm:p-7">
                ...
                <div className="mt-5 grid gap-3 sm:grid-cols-2">
                  <FlyerChecklistItem label="Event selected" done={Boolean(selectedEvent)} />
                  <FlyerChecklistItem label="Prompt added" done={Boolean(prompt.trim())} />
                  <FlyerChecklistItem label="Style selected" done={Boolean(style)} />
                  <FlyerChecklistItem label="Image uploaded" done={Boolean(imagePreview)} />
                </div>
              </div>
                  ...
                  <AnalyticsMiniCard
                    ...
                    value={
                      savedCreative?.[0]?.updatedAt
                        ? new Date(savedCreative[0].updatedAt).toLocaleDateString()
                        : "N/A"
                    }
                  />
                </div>
